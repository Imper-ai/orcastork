...
from orcastork_lite import DataPointsMerged, MergedDataPoint, NamespaceId, OperatorId, SessionCompleted, SessionId
...
def _merged(session: str, value: object, clock: FakeClock) -> DataPointsMerged:
    return DataPointsMerged(
        session_id=SessionId(session),
        namespace_id=NamespaceId('ns'),
        at=clock.now(),
        added=(MergedDataPoint(data_point_type='Ip', value=value, retrieved_by=OperatorId('op')),),
        updated=(),
        revision=1,
    )
...
async def test_redis_sink_appends_one_entry_per_event_to_the_session_stream(
    ...
    assert [fields['kind'] for _entry_id, fields in entries] == ['data_points_merged', 'session_completed']
    first = json.loads(entries[0][1]['event'])
    assert first['added'] == [{'data_point_type': 'Ip', 'value': {'a': [1, 2]}, 'retrieved_by': 'op'}]
    assert first['updated'] == [] and first['revision'] == 1 and first['session_id'] == 's1'
...
async def test_redis_sink_renders_values_json_cannot_express_and_caps_the_stream(
    ...
    payloads = [json.loads(fields['event'])['added'][0]['value'] for _entry_id, fields in entries]
