...
| `asyncio.sleep(0)` — yield to the loop | `await new Promise<void>((resolve) => setImmediate(resolve))`, in tests and test doubles only; core code never needs an explicit yield. |
