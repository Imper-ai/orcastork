 ...
 *
 * The package logs a **fixed message plus structured fields** — never an interpolated string — so
 * a record is greppable by message and filterable by field, exactly as the Python package's
 * `logger.info('Operator finished', operator_id=oid)` calls are.
 *
 * Its own copy, not a shared one: `orcastork_lite` never imports `orcastork`, and a shared helper
 * module would be the first crack in that wall.
 *
 * @module
 */
...
/** The package's logging surface — the only thing core code is allowed to log through. */
export interface Logger {
  /** Detail useful when reconstructing a run; off in most deployments. */
  debug(message: string, fields?: LogFields): void;

  /** A lifecycle fact worth keeping: a session started, an operator ran, a capability activated. */
  ...
  /** A failure the package handled (a fault-isolated operator, a sink that threw). */
 ...
 *
 * stderr rather than stdout because a library must never pollute the embedding application's data
 * output, and one line per record because that is what a log collector can parse without being
 * told anything about the package. A value that cannot be serialized (a cycle, a `bigint`) is
 * replaced rather than thrown over: a logger that throws would turn an incident into two.
 */
