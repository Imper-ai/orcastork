 ...
 *
 * A **timeout** is not a window. Debounce, retry backoff, lease renewal and the session deadline
 * are arithmetic on the injected `Clock`, so a test can fast-forward them; a timeout bounds
 * something that is genuinely in flight (an operator run, an aggregator run, a capability
 * activation, a completion-tail step) and must therefore run on real time, exactly as `wait_for`
 * does. That is what lets a test drive a `FakeClock` through a session while a 20 ms bound still
 * fires on a stub that really hangs.
 *
...
/**
 * A bounded await did not finish inside its bound.
 *
 * The counterpart of the `TimeoutError` Python's `asyncio.wait_for` raises — which is why it
 * lives here with the helper that raises it rather than in `exceptions.ts`, whose classes all
 * mirror one of the Python package's own.
 */
export class OperationTimeoutError extends OrchestrationError {}
